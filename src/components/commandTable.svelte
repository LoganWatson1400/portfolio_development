...
// directories have a non-empty "pages" object
const dirs  = pages.filter(([, v]) => v.pages && Object.keys(v.pages).length > 0);
// files are everything else
const files = pages.filter(([, v]) => !v.pages || Object.keys(v.pages).length === 0);
</script>
            ...
            <td>
                <div style="display: flex; flex-direction: column; text-align: left; gap: 8px;">
                    ...
                    {#if line.command === "cd"}
                        ...
                        <div style="display: flex; flex-direction: column; justify-content: space-evenly; margin-left: 10%;">
